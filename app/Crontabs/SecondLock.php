...
/**
 * @Crontab(name="SecondLock", rule="\/5 * * * * *", callback="execute", memo="拉黑通知")
 */
class SecondLock
{
    ...
    {
        $info = $this->crontabService->findWhere('task_name','SecondLock');
        if(!$info){
            return false;
        }
        if($info->status == 0){
        ...
        try {
            //排除带单时间
            $orderCountOne  = $this->getCache()->get('SecondLockOne_'.date('Y-m-d'));
            ...
            $second_money_lock = $this->app(SysConfigService::class)->value('second_money_lock');
            $second_number_lock = $this->app(SysConfigService::class)->value('second_number_lock');
            ...
            })->where(function ($query) use ($second_money_lock,){
                $query->whereRaw('(income_today - deficit_today) >= ' . $second_money_lock);
            })->pluck('user_id')->toArray();
            //今日盈利超过真实充值二倍，
            $orderCount1000  = $this->app(UserRewardService::class)->getQuery()->whereNotIn('user_id', function ($query){
                return $query->select('id')->from('user')->where('types',3);
            })->where(function ($query) use ($second_number_lock){
                $query->whereRaw('(income_today - deficit_today) >= ' . $second_number_lock);
            })->pluck('user_id')->toArray();
