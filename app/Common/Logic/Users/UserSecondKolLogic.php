...
use App\Common\Model\Users\UserSecondKol;


class UserSecondKolLogic extends BaseLogic
{
    /**
     * @var UserExtend
     */
    ...
    public function search(array $where){
        ...
        $query = $this->getQuery()->when(isset($where['username']) && $where['username'] !== '', function ($query) use($where){
            ...
            return $query->join('user', function ($join) use($where){
                $join->on('user_extend.user_id', '=', 'user.id')->where( function ($query) use($where){
            ...
            })->select('user_extend.*', 'user.id as user_id', 'user.username');
