...
use App\Common\Model\Users\UserPowerLog;


class UserPowerLogLogic extends BaseLogic
{
    /**
     * @var UserPowerLog
     */
    ...
    public function search(array $where){
        ...
        $query = $this->getQuery()->when(isset($where['username']) && $where['username'] !== '', function ($query) use($where){
            ...
            return $query->join('user', function ($join) use($where){
                $join->on('user_power_log.user_id', '=', 'user.id')->where( function ($query) use($where){
            ...
            })->select('user_power_log.*', 'user.id as user_id', 'user.username');
            ...
            return $query->where('user_power_log.created_at','>=',$where['timeStart'] );

        })->when(isset($where['timeEnd']) && $where['timeEnd'] !=='', function ($query)use($where){

            return $query->where('user_power_log.created_at','<',$where['timeEnd']);
